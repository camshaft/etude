...
//! Unit, oracle, and property tests for the `ByteRope` public surface and its internals.
...
/// A `ByteRope` must be no larger than the flat chunk buffer it replaces: the `Deep` variant is
/// boxed, so a rope is `Bytes + VecDeque` — the same footprint as `etude_bytevec::ByteVec`.
#[test]
fn size_matches_flat_buffer() {
    let rope = core::mem::size_of::<ByteRope>();
    let flat = core::mem::size_of::<etude_bytevec::ByteVec>();
    assert!(
        rope <= flat,
        "ByteRope ({rope}) must not exceed ByteVec ({flat})"
    );
}
...
fn single_chunk_is_flat_and_allocation_light() {
    ...
    match &rope.repr {
        Repr::Small { head, additional } => {
            ...
            assert_eq!(additional.capacity(), 0, "single chunk must not allocate a deque");
        }
...
#[test]
fn pop_front_back() {
    let mut rope: ByteRope = [chunk(b"a"), chunk(b"bb"), chunk(b"ccc")].into_iter().collect();
...
fn advance_partial_and_whole_chunks() {
    let mut rope: ByteRope =
        [chunk(b"hello"), chunk(b" "), chunk(b"world")].into_iter().collect();
...
fn promotes_and_demotes_preserving_contents() {
    ...
    assert!(rope.is_empty());
    assert!(matches!(rope.repr, Repr::Small { .. }), "should have demoted");
}
...
fn set_byte_in_both_tiers_and_cow_preserves_shared() {
    for n in [3usize, PROMOTE_AT * 3 + 11] {
        ...
        assert_eq!(snapshot, snap_bytes, "shared snapshot observed a write, n={n}");
        assert_eq!(
            rope.set_byte(expected.len(), 0),
            Err(ByteRopeError::OutOfBounds(expected.len())),
        );
    }
}
...
fn replace_in_both_tiers_and_cow_preserves_shared() {
    for n in [4usize, PROMOTE_AT * 3 + 7] {
        ...
        for i in 0..n {
            let b = [(i % 251) as u8, (i % 241) as u8, (i % 239) as u8, (i % 233) as u8];
            rope.push_back(chunk(&b));
        ...
        // Replace the tail with another ByteRope (zero-copy chunk splice).
        let vr: ByteRope = [chunk(b"aa"), chunk(b"bbb")].into_iter().collect();
        let e = model.len();
        ...
        assert!(matches!(
            ...
            Err(ByteRopeError::OutOfBounds(_))
        ));
    }
}
...
fn replace_equal_length_overwrite_is_in_place() {
    ...
    let mut rope: ByteRope = Bytes::from(vec![0u8; 1000]).into();
    ...
    assert_eq!(rope.chunks().len(), 1, "equal-length overwrite fragmented the chunk");
}

#[test]
fn replace_equal_length_overwrite_spans_chunks_in_place() {
    let mut rope: ByteRope =
        [chunk(b"aaaa"), chunk(b"bbbb"), chunk(b"cccc"), chunk(b"dddd")]
            .into_iter()
            .collect();
    ...
    assert_eq!(rope, model);
    assert_eq!(rope.chunks().len(), before, "spanning overwrite changed chunk count");
    ...
    assert_eq!(shared, b"aaaabbbbccccdddd", "snapshot observed the overwrite");
}
            ...
            for end in (start..=total).step_by(step) {
                for vlen in [0usize, 1, 5, 40] {
                    for kind in 0..3 {
                        ...
                        let val: Vec<u8> =
                            (0..vlen).map(|k| 200u8.wrapping_add(k as u8)).collect();
                        match kind {
...
fn replace_small_inserts_coalesce_and_do_not_fragment() {
    ...
    assert!(chunk_count < 40, "coalescing failed: {chunk_count} chunks for 400 bytes");
}
...
fn replace_structural_deep_tier() {
    ...
    for &(at, del, ins) in &[
        ...
        rope.replace(at..at + del, Bytes::from(val.clone())).unwrap();
...
fn replace_equal_length_overwrite_deep_tier() {
    ...
    let total = model.len();
    for &(at, len) in &[(0usize, 10usize), (total / 3, 100), (total / 2, 120), (total - 20, 20)] {
        let val: Vec<u8> = (0..len).map(|k| ((at + k) as u8) ^ 0x33).collect();
        rope.replace(at..at + len, Bytes::from(val.clone())).unwrap();
    ...
    assert_eq!(rope.chunks().len(), chunks_before, "deep overwrite changed chunk count");
...
fn deep_rope(n: usize) -> (ByteRope, Vec<u8>) {
    let mut rope = ByteRope::new();
    let mut flat = Vec::new();
    for i in 0..n {
...
fn deep_rope_with_buffered_ends() -> (ByteRope, Vec<u8>) {
...
fn trait_impls_behave() {
    ...
    assert_eq!(ByteRope::from(&b"abc"[..]), b"abc");
    assert_eq!(ByteRope::from(vec![1u8, 2, 3]), [1u8, 2, 3]);
    let v: Vec<Bytes> = ByteRope::from_iter([Bytes::from_static(b"x"), Bytes::from_static(b"y")]).into();
    assert_eq!(v.len(), 2);
}
        ...
        for op in &ops {
            match op {
                ...
                Op::Replace(a, b, d, kind) => {
                    ...
                    // another ByteRope (zero-copy chunk splice), and an equal-length overwrite
                    ...
                    match kind % 4 {
                        ...
                        2 => {
                            let vr: ByteRope =
                                repl.chunks(3).map(Bytes::copy_from_slice).collect();
...
fn set_byte_on_large_shared_chunk_splits_instead_of_copying() {
    ...
    assert_eq!(chunks.len(), 3, "chunk split into prefix / edited byte / suffix");
...
fn set_byte_small_shared_copies_whole_and_unique_edits_in_place() {
    ...
    assert_eq!(rope2.get(0).unwrap().as_ptr(), ptr, "same allocation, mutated in place");
...
fn set_byte_bounded_cow_in_tree_rebalances_and_preserves_sharing() {
    ...
    for &off in &[3usize, flat.len() / 3, flat.len() / 2, COW_SPLIT_ABOVE + 1, flat.len() - 1] {
