...
//! A [`Builder`] for efficiently constructing a [`ByteRope`], mirroring `etude-bytevec`'s `Builder`
//! so a bytevec caller can swap the two with no source changes.
...
/// A builder for efficiently constructing a [`ByteRope`] by buffering writes.
///
...
///
/// let mut builder = ByteRope::builder(1024);
...
impl Builder {
    ...
    /// use etude_byterope::ByteRope;
    ///
    /// let builder = ByteRope::builder(1024);
    /// ```
    ...
    /// let builder = ByteRope::builder(1024).with_inline_threshold(16);
    /// ```
    ...
    /// Appends the contents of another [`ByteRope`] to this builder.
    ///
    ...
    /// builder.put_slice(b"hello");
    ///
    /// let mut other = ByteRope::from(b" world");
    /// builder.append(&mut other);
    ///
    /// let result = builder.finish();
    /// assert_eq!(result, b"hello world");
    /// ```
    pub fn append(&mut self, bytes: &mut ByteRope) {
        if bytes.is_empty() {
            return;
        }
        self.flush();
    ...
    /// Appends the contents of another [`ByteRope`] to this builder, leaving the source untouched.
    ///
    /// This operation flushes the current head buffer before appending the new bytes.
    ///
    /// # Examples
    ...
    /// builder.put_slice(b"hello");
    ///
    /// let other = ByteRope::from(b" world");
    /// builder.extend(&other);
    ///
    /// let result = builder.finish();
    /// assert_eq!(result, b"hello world");
    /// ```
    pub fn extend(&mut self, bytes: &ByteRope) {
        if bytes.is_empty() {
            return;
        }
        self.flush();
    ...
    /// assert!(builder.is_empty());
    /// ```
    pub fn split(&mut self) -> ByteRope {
        self.flush();
    ...
    ///
    /// After this operation, `self` contains elements `[at, len)`, and the returned [`ByteRope`]
    /// contains elements `[0, at)`.
    ///
    /// # Errors
    ///
    /// Returns [`ByteRopeError::OutOfBounds`] if `at` is greater than the builder's length.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Finishes building and returns the constructed [`ByteRope`].
    ///
    /// This operation consumes the builder and returns the final [`ByteRope`] containing all written
    /// bytes.
    ///
    /// # Examples
    ///
...
impl From<ByteRope> for Builder {
    fn from(chunks: ByteRope) -> Self {
        Builder {
            chunks,
            head: BytesMut::new(),
...
impl From<Builder> for ByteRope {
...
mod tests {
    ...
    fn inline_threshold_compacts_small_chunks() {
        ...
        let mut b = ByteRope::builder(1024).with_inline_threshold(16);
        b.put_bytes(Bytes::from_static(b"ab"));
        b.put_bytes(Bytes::from_static(b"cd"));
        let out = b.finish();
        assert_eq!(out, b"abcd");
    ...
    fn from_rope_roundtrips_through_builder() {
        let rope = ByteRope::from(b"seed");
        let mut b = Builder::from(rope);
        b.put_slice(b"-tail");
        let out: ByteRope = b.into();
    ...
    fn out_of_bounds_split_to_errors() {
        let mut b = ByteRope::builder(1024);
        b.put_slice(b"abc");
        assert_eq!(b.split_to(4), Err(ByteRopeError::OutOfBounds(4)));
    }
}
