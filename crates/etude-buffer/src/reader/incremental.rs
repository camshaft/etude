...
impl<C: Storage> WithStorage<'_, C> {
    ...
    pub fn set_fin(&mut self) -> Result<&mut Self, Error> {
        let final_offset = self
            .incremental
            .current_offset
            .checked_add(self.buffered_len() as u64)
...
impl<C: Storage> Storage for WithStorage<'_, C> {
    ...
    fn read_chunk(&mut self, watermark: usize) -> Result<Chunk<'_>, Self::Error> {
        ...
        self.incremental.current_offset += chunk.len() as u64;
        Ok(chunk)
    }
        ...
        Dest: writer::Storage + ?Sized,
    {
        ...
        self.incremental.current_offset += chunk.len() as u64;
        self.incremental.current_offset += dest.written_len() as u64;
        Ok(chunk)
    }
...
mod tests {
    ...
    fn incremental_test() {
        ...
        assert_eq!(incremental.current_offset(), 0u64);
        ...
        assert_eq!(incremental.current_offset(), 4u64);
        ...
        let incremental = Incremental::new(100u64);
        assert_eq!(incremental.current_offset(), 100u64);
    }
}
