...
//!
//! [`ByteVec`] is a drop-in for a chunked byte buffer (a `VecDeque<Bytes>` with a cached length):
...
pub mod builder;
pub use builder::{Behavior, Builder, DefaultBehavior};

pub mod tagged;
pub use tagged::Tagged;

// ---- iterator name aliases --------------------------------------------------------------------
// The chunk and draining iterators are exposed under their historical `etude_bytevec` names in
// addition to their descriptive names, so long-standing consumers keep compiling unchanged.

/// The chunk iterator under its `ChunkIter` name (an alias of [`Chunks`]).
pub type ChunkIter<'a> = Chunks<'a>;
/// The draining chunk iterator under its `DrainIter` name (an alias of [`IntoChunks`]).
...
/// A structural [`ByteVec::replace`] whose resulting chunk would be no larger than this is collapsed
...
/// Error returned by the byte-granular operations ([`ByteVec::advance`], [`ByteVec::split_to`]).
///
/// # Examples
///
/// ```
/// use etude_bytevec::ByteVecError;
///
/// assert_eq!(ByteVecError::OutOfBounds(3).to_string(), "index out of bounds: 3");
/// assert_eq!(ByteVecError::OutOfBoundsRange(2, 5).to_string(), "range out of bounds: 2..5");
/// // Both convert to an UnexpectedEof io::Error, like ByteVecError.
/// let e: std::io::Error = ByteVecError::OutOfBoundsRange(2, 5).into();
...
pub enum ByteVecError {
    ...
    OutOfBounds(usize),
    /// A `start..end` range was past the end of the rope. (The byte-granular ops currently report
    /// [`ByteVecError::OutOfBounds`].)
    OutOfBoundsRange(usize, usize),
}

impl core::fmt::Display for ByteVecError {
...
#[cfg(feature = "std")]
impl From<ByteVecError> for std::io::Error {
    #[inline]
    fn from(error: ByteVecError) -> Self {
        match error {
            ByteVecError::OutOfBounds(_) | ByteVecError::OutOfBoundsRange(_, _) => {
...
enum Repr {
    ...
    /// large variant does not inflate every (usually `Small`) rope — it keeps `ByteVec` the same
...
impl<K> Rope<K> {
    ...
    ///
    /// const EMPTY: ByteVec = ByteVec::new();
    ...
    fn chunk_count(&self) -> usize {
        match &self.repr {
            Repr::Small { head, additional } => usize::from(!head.is_empty()) + additional.len(),
    ...
    /// (few) chunks directly. Beyond the `ByteVec` API (like [`ByteVec::slice`]) because the tree
    ...
    /// Returns [`ByteVecError::OutOfBounds`] if `at` exceeds the rope, matching the flat buffer.
    ...
    pub fn split_to(&mut self, at: usize) -> Result<Self, ByteVecError> {
        ...
        if let Some(head_bytes) = fast_head_bytes {
            ...
            let Repr::Deep(d) = taken.repr else {
                unreachable!()
            };
    ...
    /// another `ByteVec`, a [`Chain`](etude_buffer::reader::Chain), a network buffer, etc. Owned
    ...
    /// [`ByteVec::slice`]); the Cadenza runtime builds byte edits from splices.
    pub fn replace<R>(
        &mut self,
        ...
        R: etude_buffer::reader::Buffer<Error = core::convert::Infallible>,
    {
        match &mut self.repr {
            Repr::Small { head, additional } => {
                overwrite_flat(head, additional, start, count, value)
            }
...
/// Iterator over a rope's `Bytes` chunks, in order. See [`ByteVec::chunks`].
///
...
impl<'a> Chunks<'a> {
    ...
    fn next_chunk(&mut self) -> Option<&'a Bytes> {
        match &mut self.inner {
            ChunksInner::Small { head, rest } => core::mem::replace(head, rest.next()),
            ChunksInner::Deep {
                head,
                tree,
                tail,
...
fn resolve_range(
    ...
    let start = match range.start_bound() {
        ...
        Bound::Excluded(&s) => s
            .checked_add(1)
            .ok_or(ByteVecError::OutOfBounds(usize::MAX))?,
        Bound::Unbounded => 0,
    };
    let end = match range.end_bound() {
        Bound::Included(&e) => e
    ...
    if start > end || end > len {
        return Err(ByteVecError::OutOfBounds(start.max(end)));
    }
    Ok((start, end))
}
...
impl<const N: usize> PartialEq<[u8; N]> for ByteVec {
    #[inline]
...
// Chunk-aware (`memcmp` per overlapping run), not a byte-at-a-time flatten compare.
...
// ---- byte-buffer trait plumbing (etude-buffer reader/writer integration) ----
...
impl reader::Buffer for ByteVec {
...
impl bytes::Buf for ByteVec {
    ...
    fn copy_to_bytes(&mut self, len: usize) -> Bytes {
        assert!(
            len <= self.len,
            "copy_to_bytes {len} past end (len {})",
            self.len
        );
...
impl From<bytes::BytesMut> for ByteVec {
    #[inline]
...
impl From<alloc::string::String> for ByteVec {
    #[inline]
...
impl Extend<ByteVec> for ByteVec {
    #[inline]
    fn extend<I: IntoIterator<Item = ByteVec>>(&mut self, iter: I) {
...
/// Draining iterator over a rope's `Bytes` chunks (front to back). See [`ByteVec::into_iter`].
...
impl ByteVec {
    ...
    ///
    /// ```
    /// use etude_bytevec::{ByteVec, Reader};
    ///
    /// let rope = ByteVec::from(b"data");
