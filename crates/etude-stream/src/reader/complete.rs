// Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
// SPDX-License-Identifier: Apache-2.0

use crate::reader::Stream;
use etude_buffer::{
    Error,
    reader::{Buffer, Chunk},
    writer,
};

/// Wraps a single [`Buffer`] instance as a [`Stream`].
///
/// This can be used for scenarios where the entire stream is buffered and known up-front.
#[derive(Debug)]
pub struct Complete<'a, S> {
    storage: &'a mut S,
    current_offset: u64,
    final_offset: u64,
}

impl<'a, S> Complete<'a, S>
where
    S: Buffer,
{
    #[inline]
    pub fn new(storage: &'a mut S) -> Result<Self, Error> {
        let final_offset = u64::try_from(storage.buffered_len())
            .ok()
            .ok_or(Error::OutOfRange)?;
        Ok(Self {
            storage,
            current_offset: 0u64,
            final_offset,
        })
    }
}

impl<S> Buffer for Complete<'_, S>
where
    S: Buffer,
{
    type Error = S::Error;

    #[inline]
    fn buffered_len(&self) -> usize {
        self.storage.buffered_len()
    }

    #[inline]
    fn buffer_is_empty(&self) -> bool {
        self.storage.buffer_is_empty()
    }

    #[inline]
    fn read_chunk(&mut self, watermark: usize) -> Result<Chunk<'_>, Self::Error> {
        let chunk = self.storage.read_chunk(watermark)?;
        self.current_offset += chunk.len() as u64;
        Ok(chunk)
    }

    #[inline]
    fn partial_copy_into<Dest>(&mut self, dest: &mut Dest) -> Result<Chunk<'_>, Self::Error>
    where
        Dest: writer::Buffer + ?Sized,
    {
        let mut dest = dest.track_write();
        let chunk = self.storage.partial_copy_into(&mut dest)?;
        self.current_offset += chunk.len() as u64;
        self.current_offset += dest.written_len() as u64;
        Ok(chunk)
    }

    #[inline]
    fn copy_into<Dest>(&mut self, dest: &mut Dest) -> Result<(), Self::Error>
    where
        Dest: writer::Buffer + ?Sized,
    {
        let mut dest = dest.track_write();
        self.storage.copy_into(&mut dest)?;
        self.current_offset += dest.written_len() as u64;
        Ok(())
    }
}

impl<C> Stream for Complete<'_, C>
where
    C: Buffer,
{
    #[inline]
    fn current_offset(&self) -> u64 {
        self.current_offset
    }

    #[inline]
    fn final_offset(&self) -> Option<u64> {
        Some(self.final_offset)
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn read_chunk_test() {
        let mut storage: &[u8] = &[1, 2, 3, 4];
        let mut reader = Complete::new(&mut storage).unwrap();
        let mut reader = reader.with_checks();

        assert_eq!(reader.current_offset(), 0u64);
        assert_eq!(reader.final_offset(), Some(4u64));

        let chunk = reader.read_chunk(usize::MAX).unwrap();
        assert_eq!(&*chunk, &[1, 2, 3, 4]);

        assert_eq!(reader.current_offset(), 4u64);
        assert!(reader.buffer_is_empty());
    }

    #[test]
    fn partial_copy_test() {
        let mut storage: &[u8] = &[1, 2, 3, 4];
        let mut reader = Complete::new(&mut storage).unwrap();
        let mut reader = reader.with_checks();

        assert_eq!(reader.current_offset(), 0u64);
        assert_eq!(reader.final_offset(), Some(4u64));

        let mut dest: &mut [u8] = &mut [0; 4];
        let chunk = reader.partial_copy_into(&mut dest).unwrap();
        assert_eq!(&*chunk, &[1, 2, 3, 4]);

        assert_eq!(reader.current_offset(), 4u64);
        assert!(reader.buffer_is_empty());
    }

    #[test]
    fn copy_test() {
        let mut storage: &[u8] = &[1, 2, 3, 4];
        let mut reader = Complete::new(&mut storage).unwrap();
        let mut reader = reader.with_checks();

        assert_eq!(reader.current_offset(), 0u64);
        assert_eq!(reader.final_offset(), Some(4u64));

        let mut dest = [0; 4];
        {
            let mut dest = &mut dest[..];
            reader.copy_into(&mut dest).unwrap();
        }
        assert_eq!(&dest[..], &[1, 2, 3, 4]);

        assert_eq!(reader.current_offset(), 4u64);
        assert!(reader.buffer_is_empty());
    }
}
